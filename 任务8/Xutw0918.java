...
public class Xutw0918 extends JFrame implements ActionListener {
	 ...
	 private JPanel contentPane;
	    ...
	    JLabel Changelabel=new JLabel("修改值");
	    JButton putinChange=new JButton("修改");
	    int Select_row=0;
	    int Select_col=0;
	    ...
	    JTextField ChangeText=new JTextField();//修改属性值
		/**
	    ...
	    public Xutw0918() {
 		...
 		table.setRowSelectionAllowed(true);//设置是否可以选择此模型中的行
	        table.setColumnSelectionAllowed(true);//设置是否可以选择此模型中的列
	        ...
	        frame.add(putinChange);
	        frame.add(ChangeText);
	        ...
	        putinChange.addActionListener(this);
			//Select_col=tm.getColll();
			//Select_row=tm.getRowww();
			//tm.TableRowCol(table);
	        //b
	      ...
	      //布局
	        ...
	        Changelabel.setBounds(90, 30, 70, 30);
	        ChangeText.setBounds(165,30,150,30);
	        putinChange.setBounds(320,30,100,30);
			...
			scrollPane.setBounds(50,75,(int)windowWidth-100,(int)windowHeight-400);
	    ...
	    public void actionPerformed(ActionEvent e) {
	        ...
	        if(e.getSource()==putinupdate) {
				...
				if(e.getSource()==putinChange) {
					tm.TableRowCol(table);
					Select_col=tm.getColll();
					Select_row=tm.getRowww();
					System.out.println(Select_col);
					System.out.println(Select_row);
					if(Select_col==-1&&Select_row==-1) {
						JOptionPane.showMessageDialog(null,"请先选择单元格","修改信息失败",0);
					}else try{
						String change_name=head[Select_col];
						String change_value=ChangeText.getText();
						String ID=(String) table.getValueAt(Select_row,0);
						String sql;
						if(change_value.equals("空"))
						{
							sql = "UPDATE TEMPL SET " + change_name + " = " + null + " WHERE EMPNO" + " = "+ID;
						}else {
